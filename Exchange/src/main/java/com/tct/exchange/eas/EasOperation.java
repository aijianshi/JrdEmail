 ...
 *BUGFIX-890079  2014/01/09   zhaotianyong    [HOMO][HOMO][HOMO][TMO 60073]Insufficient warning message when the Exchange email account is setup with untrusted SSL certificate
 ...
 *BUGFIX-1128014 2015-12-30    jian.xu        [Crash][Stability][Monitor]During stability test, in wifi test case, the sdevice crash
...
import com.tct.exchange.service.EasServerConnection;
 ...
 * {@link com.tct.emailcommon.service.IEmailService} message handlers. In such cases, due to
 * how {@link com.tct.exchange.service.EasService} uses this class, the subclass needs to
...
public abstract class EasOperation {
    ...
    /** validate certificate failed*/
    public static final int RESULT_CERTIFICATE_VALIDATE_ERROR = -101;
    ...
    public boolean init(final boolean allowReload) {
        if (mAccount == null || allowReload) {
            //TS: jian.xu 2015-12-30 EMAIL BUGFIX-1128014 MOD_S
            //Note: Maybe throw exception while restore account, catch it.
            try {
                mAccount = Account.restoreAccountWithId(mContext, getAccountId());
            } catch (ProviderUnavailableException e) {
                LogUtils.w(LOG_TAG, "restore account failed, throw ProviderUnavailableException...");
            }
            if (mAccount != null) {
    ...
    public int performOperation() {
        ...
        do {
            ...
            // TS: zhaotianyong 2014-01-09 EXCHANGE BUGFIX-890079 ADD_S
            } catch (final SSLException e){
                return RESULT_CERTIFICATE_VALIDATE_ERROR;
            }
           // TS: zhaotianyong 2014-01-09 EXCHANGE BUGFIX-890079 ADD_E
            ...
            try {
                ...
                if (response.isSuccess()) {
                    ...
                    try {
                        ...
                        //TS: jin.dong 2016-3-24 EMAIL BUGFIX_1567481 MOD_S
                        //note: convert into other failure instead of network error to avoid confusion.
                        return RESULT_OTHER_FAILURE;
                        //TS: jin.dong 2016-3-24 EMAIL BUGFIX_1567481 MOD_E
                ...
                LogUtils.i(LOG_TAG,"EasOperation performOperation result is: %d", result);
                ...
                //TS: yanhua.chen 2016-3-14 EMAIL BUGFIX_1719263 ADD_S
                //Handle attachment internal error.
                if (result == EasLoadAttachment.RESULT_ATTACHMENT_INTERNAL_HANDLING_ERROR) {
                    return result;
                }
                //TS: yanhua.chen 2016-3-14 EMAIL BUGFIX_1719263 ADD_E
     ...
     * @return The {@link HttpEntity} to pass to {@link com.tct.exchange.service.EasServerConnection#makePost}.
     * @throws IOException
     */
...
//        if (PermissionUtil.checkPermissionAndLaunchExplain(context, Manifest.permission.READ_PHONE_STATE)) {
//            tm = (TelephonyManager) context.getSystemService(
//                    Context.TELEPHONY_SERVICE);
//        } else {
//            LogUtils.i(PermissionUtil.TAG,"permission %s needed in expandedAddDeviceInformationToSerializer()",Manifest.permission.READ_PHONE_STATE);
//            tm = null;
//        }
...
//        }
        ...
        final String deviceId = Device.getMiddleManDeviceId(context);
        final String phoneNumber = Device.getLine1Number(context);
        final String operator;
        final String operatorName = Device.getNetworkOperatorName(context);
        final String operatorNumber = Device.getNetworkOperator(context);
        if (!TextUtils.isEmpty(operatorName) && !TextUtils.isEmpty(operatorNumber)) {
            operator = operatorName + " (" + operatorNumber + ")";
        } else if (!TextUtils.isEmpty(operatorName)) {
            operator = operatorName;
        ...
        if (deviceId != null) {
            s.data(Tags.SETTINGS_IMEI, deviceId);   //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 MOD
        }
    ...
    public static int translateSyncResultToUiResult(final int result) {
        switch (result) {
              case RESULT_TOO_MANY_REDIRECTS:
                return UIProvider.UIPROVIDER_LASTSYNCRESULTAUTH_INTERNAL_ERROR;
            case RESULT_NETWORK_PROBLEM:
                return UIProvider.UIPROVIDER_LASTSYNCRESULTAUTH_CONNECTION_ERROR;
            ...
            case RESULT_CLIENT_CERTIFICATE_REQUIRED:
                return UIProvider.UIPROVIDER_LASTSYNCRESULT_AUTH_ERROR;
        ...
        return UIProvider.UIPROVIDER_LASTSYNCRESULT_SUCCESS;
    }
}
