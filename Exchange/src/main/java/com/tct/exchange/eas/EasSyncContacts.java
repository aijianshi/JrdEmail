...
*BUGFIX-1063132 2015/08/07   chaozhang       [jrdlogger]com.tct.exchange Java (JE)
...
import com.tct.exchange.adapter.ContactsSyncParser;
...
public class EasSyncContacts extends EasSyncCollectionTypeBase {
    ...
    private static final int MAX_UPSYNC_ITEM_COUNT = 200;      // TS: zheng.zou 2015-06-12 EXCHANGE BUGFIX-1022349 ADD
    ...
    public void setSyncOptions(final Context context, final Serializer s,
        ...
        setUpsyncCommands(context, s, context.getContentResolver(), account, mailbox, protocolVersion);  //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 MOD
    }
    ...
    private void dirtyContactsWithinDirtyGroups(final ContentResolver cr, final Account account) {
        if (!PermissionUtil.checkPermissionAndLaunchExplain(mContext,Manifest.permission.READ_CONTACTS)){
            LogUtils.i(PermissionUtil.TAG, "permission %s needed in dirtyContactsWithinDirtyGroups()", Manifest.permission.READ_CONTACTS);
            return;
        }
    ...
    // note: judge if the local side have contact change
    @Override
    public boolean hasLocalChange(Context context, Account account) {
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_S
        if (!PermissionUtil.checkPermissionAndLaunchExplain(context, Manifest.permission.READ_CONTACTS)){
            LogUtils.i(PermissionUtil.TAG,"permission %s needed in EasSyncContacts hasLocalChange()", Manifest.permission.READ_CONTACTS);
            return false;
        }
        ...
        return ret;
        //TS: jin.dong 2015-07-23 EMAIL BUGFIX_1046591 MOD_E
    }
    // TS: zheng.zou 2015-06-12 EXCHANGE BUGFIX-1022349 ADD_E

    private void setUpsyncCommands(Context context, final Serializer s, final ContentResolver cr,
            final Account account, final Mailbox mailbox, final double protocolVersion)
            throws IOException {
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_S
        if (!PermissionUtil.checkPermissionAndLaunchExplain(context, Manifest.permission.READ_CONTACTS)){
            LogUtils.i(PermissionUtil.TAG,"permission %s needed in EasSyncContacts setUpsyncCommands()", Manifest.permission.READ_CONTACTS);
            return;
        }
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_E
        ...
        try {
            ...
            int itemCount = 0;      // TS: zheng.zou 2015-06-12 EXCHANGE BUGFIX-1022349 ADD_S
            while (ei.hasNext()) {
                // TS: zheng.zou 2015-06-12 EXCHANGE BUGFIX-1022349 ADD_S
                // note : the server side has a limit of the numbers of the item posted,
                // in standard exchange server, the limit is 200
                itemCount++;
                if (itemCount > MAX_UPSYNC_ITEM_COUNT) {
                    LogUtils.i(TAG,"the number of local contact change has exceed MAX_UPSYNC_ITEM_COUNT");
                    break;
                }
                // TS: zheng.zou 2015-06-12 EXCHANGE BUGFIX-1022349 ADD_E
                ...
                if (!hasSetFileAs) {
                    ...
                    } finally {// TS: chaozhang 2015-08-07 BUGFIX-1063132 MOD_S
                        //judge the null cursor to avoid NPE
                        if (c != null) {
                            c.close();
                        }// TS: chaozhang 2015-08-07 BUGFIX-1063132 MOD_S
                    }
                    if (fileAs == null) {
                ...
                if (!groupIds.isEmpty()) {
                    ...
                    for (final int id: groupIds) {
                        ...
                        try {
                            ...
                            if (c != null) {// TS: chaozhang 2015-08-07 BUGFIX-1063132 MOD_S
                                //judge the null cursor to avoid NPE
                                c.close();
                            }// TS: chaozhang 2015-08-07 BUGFIX-1063132 MOD_E
                        }
                    }
                    if (!groupFirst) {
                        s.end();
        ...
        } catch (SQLException e) {// TS: chaozhang 2015-08-07 BUGFIX-1063132 MOD_S
            //if Exchange has been uninstalled,issue will hapen if still have some sync.
            LogUtils.d(TAG, "Exception happen during setUpsyncCommands:", e);
        }// TS: chaozhang 2015-08-07 BUGFIX-1063132 MOD_E
        finally {
            ei.close();
        }
    ...
    public void cleanup(final Context context, final Account account) {
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_S
        if (!PermissionUtil.checkPermissionAndLaunchExplain(context, Manifest.permission.WRITE_CONTACTS)){
            LogUtils.i(PermissionUtil.TAG,"permission %s needed in EasSyncContacts cleanup()", Manifest.permission.WRITE_CONTACTS);
            return;
        }
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_E
    ...
    public static void wipeAccountFromContentProvider(final Context context,
        ...
        try {
            ...
            if (PermissionUtil.checkPermissionAndLaunchExplain(context,Manifest.permission.WRITE_CONTACTS)){
                ...
                LogUtils.i(PermissionUtil.TAG, "permission %s needed in EasSyncContacts wipeAccountFromContentProvider()", Manifest.permission.WRITE_CONTACTS);
            }
