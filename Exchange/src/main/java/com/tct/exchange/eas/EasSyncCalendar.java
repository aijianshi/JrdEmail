 ...
 *============== ============ =============== ==============================
 *CONFLICT-50011 2014/10/24   zhaotianyong     Modify the package conflict
...
import com.tct.exchange.adapter.CalendarSyncParser;
...
//TS: MOD by zhaotianyong for CONFLICT_50011 START
//import com.android.calendarcommon2.DateException;
//import com.android.calendarcommon2.Duration;
import com.tct.fw.ex.calendarcommon2.DateException;
import com.tct.fw.ex.calendarcommon2.Duration;
//TS: MOD by zhaotianyong for CONFLICT_50011 END
...
public class EasSyncCalendar extends EasSyncCollectionTypeBase {
    ...
    public EasSyncCalendar(final Context context, final Account account,
        ...
        //chceck Manifest.permission.READ_CALENDAR in instance creation
    ...
    /** Get calendar filter */
    private String getCalendarFilter(final Account account) {
        if (account == null) {
            return Eas.FILTER_2_WEEKS;
        }

        final int syncLookback = account.getSyncCalendarLookback();
        switch (syncLookback) {
            case SyncCalendarWindow.SYNC_CALENDAR_WINDOW_2_WEEKS:
                return Eas.FILTER_2_WEEKS;
            case SyncCalendarWindow.SYNC_CALENDAR_WINDOW_1_MONTH:
                return Eas.FILTER_1_MONTH;
            case SyncCalendarWindow.SYNC_CALENDAR_WINDOW_3_MONTH:
                return Eas.FILTER_3_MONTHS;
            case SyncCalendarWindow.SYNC_CALENDAR_WINDOW_6_MONTH:
                return Eas.FILTER_6_MONTHS;
            case SyncCalendarWindow.SYNC_CALENDAR_WINDOW_ALL:
    ...
    public void setSyncOptions(final Context context, final Serializer s,
        ...
        if (isInitialSync) {
            ...
            //NOTE: add filter calendar conditional
            setNonInitialSyncOptions(s, getCalendarFilter(account), numWindows, protocolVersion);
    ...
    //TS: junwei-xu 2015-09-29 EMAIL FEATURE-1093309 MOD_S
    //NOTE: Add  filter parameter for this method
    protected void setNonInitialSyncOptions(final Serializer s, final String filter, final int numWindows,
        ...
        setPimSyncOptions(s, filter, protocolVersion,
    ...
    private void markParentsOfDirtyEvents(final Context context, final Account account,
        ...
        if (!PermissionUtil.checkPermissionAndLaunchExplain(context,Manifest.permission.READ_CALENDAR)){
            LogUtils.i(PermissionUtil.TAG, "permission %s needed in markParentsOfDirtyEvents()", Manifest.permission.READ_CALENDAR);
            return;
        }
    ...
    private void sendEvent(final Context context, final Account account, final Entity entity,
        final String clientId, final double protocolVersion, final Serializer s)
            throws IOException {
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_S
        if (!PermissionUtil.checkPermissionAndLaunchExplain(context, Manifest.permission.READ_CALENDAR)) {
            LogUtils.i(PermissionUtil.TAG,"no READ_CALENDAR in sendEvent()");
            return;
        }
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_E
    ...
    private void handleExceptionsToRecurrenceRules(final Serializer s, final Context context,
        ...
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_S
        if (!PermissionUtil.checkPermissionAndLaunchExplain(context, Manifest.permission.WRITE_CALENDAR)) {
            LogUtils.i(PermissionUtil.TAG,"no WRITE_CALENDAR in handleExceptionsToRecurrenceRules()");
            return;
        }
        //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 ADD_E
        final ContentResolver cr = context.getContentResolver();
      ...
      //[BUGFIX]-Mod-BEGIN by TCTNB.caixia.chen,12/26/2014,PR 883989,close cursor
        try {
    ...
    private void updateAttendeesAndSendMail(final Context context, final Account account,
        ...
        if (selfOrganizer && (getInt(entityValues, Events.DIRTY) == 1)) {
            ...
            //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 MOD_S
            if (PermissionUtil.checkPermissionAndLaunchExplain(context,Manifest.permission.WRITE_CALENDAR)) {
            ...
            //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 MOD_E
    ...
    protected void setUpsyncCommands(Context context, final Account account,
        ...
        //TS: zheng.zou 2015-11-25 EMAIL TASK_996919 ADD_S
        if (!PermissionUtil.checkPermissionAndLaunchExplain(context, Manifest.permission.READ_CALENDAR)){
            LogUtils.i(PermissionUtil.TAG,"permission %s needed in EasSyncCalendar setUpsyncCommands()",Manifest.permission.READ_CALENDAR);
            return;
        }
        //TS: zheng.zou 2015-11-25 EMAIL TASK_996919 ADD_E
     ...
     */
    //TS: chao-zhang 2015-06-30 EMAIL BUGFIX-1005857 MOD_S
    //NOTE: During wiping the calendar data,we only wipe the relatived canendar.not all.
    public static void wipeAccountFromContentProvider(final Context context,
            final String emailAddress,final long calendarId) {
        String query = Calendars.ACCOUNT_NAME + "=" + DatabaseUtils.sqlEscapeString(emailAddress)
                + " AND " + Calendars.ACCOUNT_TYPE + "=" + DatabaseUtils.sqlEscapeString(
                        context.getString(R.string.account_manager_type_exchange));
        if (calendarId > 0) {
            query += " AND " + Calendars._ID + "=" + calendarId;
        }
        LogUtils.d(TAG, "wipe calendar data and the SQL query is %s",query);
        try {
            //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 MOD_S
            if (PermissionUtil.checkPermissionAndLaunchExplain(context,Manifest.permission.WRITE_CALENDAR)){
                ...
                LogUtils.i(PermissionUtil.TAG, "permission %s needed in EasSyncCalendar wipeAccountFromContentProvider()", Manifest.permission.WRITE_CALENDAR);
            }
            //TS: zheng.zou 2015-11-27 EMAIL TASK_869664 MOD_E
