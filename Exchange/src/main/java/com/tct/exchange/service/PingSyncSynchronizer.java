...
/* ----------|----------------------|----------------------|----------------- */
/* 01/08/2015|     caixia.chen      |      PR 873581       |[10776 DDA][A85E- */
/*           |                      |                      |xchangeActiveSyn- */
/*           |                      |                      |c][GSM-BTR-1-640- */
/*           |                      |                      |2] Synchronizati- */
/*           |                      |                      |on                */
...
public class PingSyncSynchronizer {
    ...
    private class AccountSyncState {
        ...
        public void syncStart() throws InterruptedException {
            ...
            if (mPingTask != null) {
                ...
                FileUtils.appendLog(TAG, "PSS Sync is pre-empting a ping acct:%d", mAccountId);
                mPingTask.stop();
            }
            if (mPingTask != null || mSyncCount > 1) {
                // There’s something we need to wait for before we can proceed.
                //TS: chao-zhang 2015-9-28 EMAIL BUGFIX_1010924 DEL_S
                //NOTE: Just thrown InterruptedException to EasService#doOperation,we must handle the exception.
                mCondition.await();
                /*try {
                    LogUtils.i(TAG, "PSS Sync needs to wait: Ping: %s, Pending tasks: %d acct: %d",
                            mPingTask != null ? "yes" : "no", mSyncCount, mAccountId);
                    FileUtils.appendLog(TAG, "PSS Sync needs to wait: Ping: %s, Pending tasks: %d acct: %d",
                            mPingTask != null ? "yes" : "no", mSyncCount, mAccountId);
                    mCondition.await();
                }
                ...
                //TS: chao-zhang 2015-9-28 EMAIL BUGFIX_1010924 DEL_E
            }
        }
        ...
        public boolean syncEnd(final boolean lastSyncHadError, final Account account,
            ...
            --mSyncCount;
            if (mSyncCount > 0) {
                LogUtils.i(TAG, "PSS Signalling a pending sync to proceed acct:%d. Pending tasks: %d",
                        account.getId(), mSyncCount);
                FileUtils.appendLog(TAG, "PSS Signalling a pending sync to proceed acct:%d. Pending tasks: %d",
                        account.getId(), mSyncCount);
                mCondition.signal();
                return false;
            } else {
                if (mPushEnabled == PUSH_UNKNOWN) {
                    ...
                    FileUtils.appendLog(TAG, "PSS push enabled is unknown");
                ...
                if (mPushEnabled == PUSH_ENABLED) {
                    if (lastSyncHadError) {
                        LogUtils.i(TAG, "PSS last sync had error, scheduling delayed ping acct:%d.",
                                account.getId());
                        FileUtils.appendLog(TAG, "PSS last sync had error, scheduling delayed ping acct:%d.",
                                 account.getId());
                        ...
                        LogUtils.i(TAG, "PSS last sync succeeded, starting new ping acct:%d.",
                                account.getId());
                        FileUtils.appendLog(TAG, "PSS last sync succeeded, starting new ping acct:%d.",
                                 account.getId());
            ...
            FileUtils.appendLog(TAG, "PSS no push enabled acct:%d.", account.getId());
            return true;
        }
         ...
         * @return Whether this account is now idle.
         */
        private boolean pingEnd(final android.accounts.Account amAccount,final int pingStatus) {
            mPingTask = null;
            if (mSyncCount > 0) {
                ...
                if (mPushEnabled == PUSH_ENABLED || mPushEnabled == PUSH_UNKNOWN) {
                    ...
                    // NOTE:After pingStatus equals STATUS_BAD_PING we reassigned,means we have tried at last 10
                    // times for pingAgagin, For Better data and power consume,just end it now 
                    //and DO NOT rePing again immediately!!
                    if (pingStatus == PingParser.STATUS_BAD_PING) {
                        LogUtils.e(TAG,"PSS pingEnd, with Bad ping,bye-bye");
                        return true;
                    }
    ...
    //NOTE: Just thrown InterruptedException to EasService#doOperation,and will handle it,do worry about.
    public void syncStart(final long accountId) throws InterruptedException {
        ...
        try {
            ...
            FileUtils.appendLog(TAG, "PSS syncStart for account acct:%d", accountId);
  ...
  //TS: chao-zhang 2015-9-28 EMAIL BUGFIX_1010924 MOD_E
    public void syncEnd(final boolean lastSyncHadError, Account account, final long accountID) {
        mLock.lock();
        try {
            // fix force close
            // ccx:fix issue: wake mCondition to avoid always waiting in syncStart and can not start ping for this account
            // maybe there are other way to fix this issue (account = null)
            if (account == null) {
                LogUtils.i(TAG, "PSS syncEnd for account = null , accountID : %d", accountID);
                account = Account.restoreAccountWithId(this.getContext(), accountID);
                if (account == null) {
                    LogUtils.i(TAG, "PSS syncEnd for account22222 = null , accountID : %d", accountID);
                    return;
                }
            }
            final long accountId = account.getId();
            LogUtils.i(TAG, "PSS syncEnd for account acct:%d", account.getId());
            FileUtils.appendLog(TAG, "PSS syncEnd for account acct:%d", account.getId());
    ...
    //TS: chaozhang 2015-07-23 EMAIL BUGFIX_1049125 MOD_S
    public void pingEnd(final long accountId, final android.accounts.Account amAccount,final int pingStatus) {
        mLock.lock();
        try {
            ...
            if (accountState.pingEnd(amAccount,pingStatus)) {
                removeAccount(accountId);
            }
        } finally {
            mLock.unlock();
