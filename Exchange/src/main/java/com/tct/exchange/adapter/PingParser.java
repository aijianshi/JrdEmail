...
public class PingParser extends Parser {
    ...
    /** Indicates that bad ping happend cause of status code STATUS_REQUEST_INCOMPLETE or STATUS_REQUEST_MALFORMED. */
    public static final int STATUS_BAD_PING = 9;
    /** Indicates the max retryTimes after bad ping*/
    public static final int MAX_TIMES_RETRY_WITH_BAD_PING = 10;
    ...
    //TS: chaozhang 2015-07-23 EMAIL BUGFIX_1049125 ADD_S
    /**
     * We must limit the rePing times,For better data/power consume.
     * @param pingStatus
     * @param tryTimes
     * @return
     */
    public static boolean shouldPingAgain(final int pingStatus, final int tryTimes) {
        LogUtils.d(TAG,
                "Need confirm should pingAgain,the pingStatus: %d,and tryTime :%d",
                pingStatus, tryTimes);
        if ((pingStatus == STATUS_REQUEST_INCOMPLETE || pingStatus == STATUS_REQUEST_MALFORMED)
                && tryTimes > MAX_TIMES_RETRY_WITH_BAD_PING) {
            LogUtils.e(TAG,"Bad Ping happened,exceend the MAX 10 frequency,must end the loop!");
            return false;
        }
        return pingStatus == STATUS_EXPIRED
                || pingStatus == STATUS_REQUEST_INCOMPLETE
    ...
    //TS: chaozhang 2015-07-23 EMAIL BUGFIX_1049125 ADD_E
    /**
