...
import com.tct.exchange.adapter.AbstractSyncAdapter.Operation;
...
public class CalendarSyncParser extends AbstractSyncParser {
    ...
    //Open the log,we want know some invaild events.
...
//                    break;
    ...
    public void commit() throws IOException {
        ...
        if (!PermissionUtil.checkPermissionAndLaunchExplain(mContext, Manifest.permission.WRITE_CALENDAR)) {
            LogUtils.i(PermissionUtil.TAG, "permission %s needed in CalenderSyncParser commit()", Manifest.permission.WRITE_CALENDAR);
            return;
        }
    ...
    protected static ContentProviderResult[] safeExecute(final ContentResolver contentResolver,
        ...
        LogUtils.e(TAG, "Try to execute "+ ops.size()+",CPO's for " + authority);
        ...
        try {
            ...
            userLog("Transaction too large; spliting!");
            ...
            for (Operation op: ops) {
                if (op.mSeparator) {
                    ...
                    userLog("Try mini-batch of ", mini.size(), " CPO's");
    ...
    protected void wipe() {
        ...
        //TS: chao-zhang 2015-06-30 EMAIL BUGFIX-1005857 ADD_S
        //Happen wipe,only wipe the current calendar,not wipe all calendar data.
        LogUtils.w(TAG, "Wiping calendar for calendar %d", mCalendarId);
        EasSyncCalendar.wipeAccountFromContentProvider(mContext,
                mAccount.mEmailAddress,mCalendarId);
        //TS: chao-zhang 2015-06-30 EMAIL BUGFIX-1005857 END_S
    }
}
