...
/* ----------|----------------------|----------------------|----------------- */
/* 10/24/2014|     zhujian.shao     |      FR 736411       |Exchange Active Sync*/
/*           |                      |porting from(FR622697)|    Priority      */
/*           |                      |                      |                  */
/* ----------|----------------------|----------------------|----------------- */
/*
 ==========================================================================
 ...
 *BUGFIX-888753  2015/03/17   zhaotianyong     [Android5.0][Exchange]The picture order in 163 and 126 account is wrong
 *BUGFIX-967903  2015/04/10   zhaotianyong    [REG][Android5.0][Exchange]The picture order is wrong in exchange accounts.
 *BUGFIX-969850  2015/04/16   gengkexue       [HOMO][ALWE] Corporate email synchronized again all folders instead of updating only new emails
 ...
 *BUGFIX-1052751 2015/07/28   junwei-xu       [FT][HK][Email]UE return to homescreen automatically when Email is in use.
...
public class EmailSyncParser extends AbstractSyncParser {
    ...
    //TS: junwei-xu 2015-07-28 EMAIL BUGFIX-1052751 MOD_S
    //Note: ensure binder transaction buffer limited in 1M, decrease the number for one batch peration
    //private static final int MAX_OPS_PER_BATCH = 50;
    private static final int MAX_OPS_PER_BATCH = 45;
    //TS: junwei-xu 2015-07-28 EMAIL BUGFIX-1052751 MOD_E
    ...
    public void addData(EmailContent.Message msg, int endingTag) throws IOException {
        ...
        while (nextTag(endingTag) != END) {
            switch (tag) {
                ...
                case Tags.EMAIL_IMPORTANCE:
                    int importance = 1;
                     // In EAS protocol, 0 Low, 1 Normal, 2 High
                     // In local database we use 2 Low, 1 Normal, 0 High
                    int priorityidxs [] = {2,1,0};
                    int tagValue = getValueInt();
                    if (tagValue == 0 || tagValue == 2 )
                    importance = priorityidxs[tagValue];
                    msg.mPriority = importance;
                    break;
        ...
        boolean mDownloadAllBody = mAccount.getDownloadOptions() == Utility.ENTIRE_MAIL ? true
                : false;
        if (atts.size() > 0) {
            msg.mAttachments = atts;
          //TS: Gantao 2015-10-15 EMAIL FEATURE_526529 ADD_S
            for (Attachment att : atts) {
                if(att.mIsInline != Utility.INLINE_ATTACHMENT && mDownloadAllBody) {
                    msg.mFlagAttachment = true;
                    break;
                }
            }
          //TS: Gantao 2015-10-15 EMAIL FEATURE_526529 ADD_E
        }

        if ((msg.mFlags & EmailContent.Message.FLAG_INCOMING_MEETING_MASK) != 0) {
            // TS: junwei-xu 2015-09-01 EMAIL BUGFIX_537843 MOD_S
            //Note: Invite message does not need to show message body, set body to null.
            // And set flag loaded to loaded complete.
            /*
            ...
            */
            ...
            // TS: junwei-xu 2015-09-01 EMAIL BUGFIX_537843 MOD_E
        }
    }
        ...
        if ((fileName != null) && (length != null) && (location != null)) {
            ...
            if (isInline && !TextUtils.isEmpty(contentId)) {
             ...
             // TS: Gantao 2015-09-19 EMAIL BUGFIX_570084 ADD_S
                ...
                att.mIsInline = Utility.INLINE_ATTACHMENT;
            ...
            atts.add(att);
          //TS: Gantao 2015-10-15 EMAIL FEATURE_526529 DEL
            //Not set flagAttachment here
//          //TS: Gantao 2015-09-28 EMAIL FEATURE_526529 MOD_S
//            if (mDownloadAllBody && att.mIsInline != Utility.INLINE_ATTACHMENT) {//We do not think the inline images is an attachment from now.
//              //TS: Gantao 2015-09-28 EMAIL FEATURE_526529 MOD_E
//                msg.mFlagAttachment = true;
//            } else {
//                // We hince that the attachment not download. we start
//                // download the attachment in EmailProvider after user click the downloadRemain
//                // button.
...
//            }
    ...
    /* (non-Javadoc)
     * @see com.tct.exchange.adapter.EasContentParser#commandsParser()
     */
    @Override
    ...
    // AM: Kexue.Geng 2015-04-16 EMAIL BUGFIX_969850 MOD_E
    ...
    public void commitImpl(int maxOpsPerBatch)
        ...
        for (EmailContent.Message msg: newEmails) {
            // TS: zhaotianyong 2015-04-10 EMAIL BUGFIX-967903 DEL
            msg.addSaveOps(ops);
            //TS: junwei-xu 2015-09-17 EMAIL BUGFIX-569939 ADD-S
            msg.addInnerRecipinetsOps(mContext, ops);
