...
public class MessageModification {
    ...
    /**
     * Sets the message's reply-to address.Only valid for drafts.
     * @param values the ContentValues that will be used to create or update the
     *            message
     * @param replyToAddress the new reply-to address
     */
    public static void putRepylToAddress(ContentValues values, String replyToAddress) {
        if(replyToAddress == null) {
            //Don't put null values.
            replyToAddress = "";
        }
        values.put(MessageColumns.REPLY_TO, replyToAddress);
    }
    ...
    public static void putPriority(ContentValues values,int priority){
        values.put(MessageColumns.MESSAGE_FLAG_PRIORITY_COLUMN,  priority);
