...
public class ObjectCursorLoader<T> extends AsyncTaskLoader<ObjectCursor<T>> {
    ...
    public ObjectCursor<T> loadInBackground() {
        // TS: zhaoxing 2016-03-03 EMAIL BUGFIX-1718620 ADD_S
        Cursor inner = null;
        try {
            inner = getContext().getContentResolver().query(mUri, mProjection,
                    mSelection, mSelectionArgs, mSortOrder);
        }catch (SQLiteException e){
            e.printStackTrace();
        }
        // TS: zhaoxing 2016-03-03 EMAIL BUGFIX-1718620 ADD_E
        if (inner == null) {
