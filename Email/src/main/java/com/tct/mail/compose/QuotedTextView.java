 ...
 *============== ============ =============== ==============================
 *BUGFIX-923740  2015/02/04   chenyanhua      [Android5.0][Email] 'From,Data,Subject,To,Cc' displays when forward a mail
...
class QuotedTextView extends LinearLayout implements OnClickListener {
    ...
    public void setQuotedText(int action, Message refMessage, boolean allow) {
        ...
        if (action == ComposeActivity.REPLY || action == ComposeActivity.REPLY_ALL) {
            ...
            //NOTE: Add header message when forward mail.
            quotedText.append(String.format(resources.getString(R.string.forward_attribution_with_headers),
                    "<br/>",
                    Utils.cleanUpString(refMessage.getFrom(), true),
                    "<br/>",
                    dateFormat.format(date),
                    "<br/>",
                    TextUtils.isEmpty(refMessage.subject) ? "" : refMessage.subject,
                    "<br/>",
                    Utils.cleanUpString(refMessage.getTo(), true),
                    "<br/>",
                    Utils.cleanUpString(refMessage.getCc(), true)));
