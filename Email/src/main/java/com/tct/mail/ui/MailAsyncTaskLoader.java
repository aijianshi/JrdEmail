 ...
 *
 * Subclasses need to implement {@link MailAsyncTaskLoader#loadInBackground()} to perform the actual
 * background task, and {@link MailAsyncTaskLoader#onDiscardResult(T)} to clean up previously loaded
 * results.
 */
...
public abstract class MailAsyncTaskLoader<T> extends AsyncTaskLoader<T> {
    ...
    public MailAsyncTaskLoader(final Context context) {
        super(context);
    }
     ...
     *
     * @param result The value returned from {@link MailAsyncTaskLoader#loadInBackground()} which
     *               is to be discarded.
     */
