...
*============== ============ =============== ==============================
*BUGFIX_876383  2014/12/18   zhaotianyong    [Android][Exchange]The font of received mails is too small
...
*BUGFIX-1489655 2015/01/23   jian.xu         [Email]The head portrait of sender will not display after add the sender as contact.
============================================================================
*/
...
import com.tct.mail.providers.ListParams;
import com.tct.mail.providers.Settings;
import com.tct.mail.providers.UIProvider;
import com.tct.mail.providers.UIProvider.CursorStatus;
...
public abstract class AbstractConversationViewFragment extends Fragment implements
    ...
    private boolean isEasCall = false; // TS: gangjin.weng 2015-04-20 EMAIL BUGFIX_940964 ADD
    ...
    protected void markUnread() {
        LogUtils.i(LOG_TAG,"markUnread()"); // TS: zheng.zou 2015-1-18 EMAIL BUGFIX_1130225 ADD
    ...
    public void onCreate(Bundle savedState) {
        ...
        if (savedState != null) {
            ...
            // TS: zhaotianyong 2014-12-18 EMAIL BUGFIX_876383 MOD_S
            mHasConversationTransformBeenReverted = false;
            // TS: zhaotianyong 2014-12-18 EMAIL BUGFIX_876383 MOD_E
        }
    }
    ...
    public ContactLoaderCallbacks getContactInfoSource() {
        if (mContactLoaderCallbacks == null) {
            ...
            //TS: jian.xu 2016-01-23 EMAIL BUGFIX-1489655 ADD_S
            mContactLoaderCallbacks.registerObserver(mContactInfoObserver);
            //TS: jian.xu 2016-01-23 EMAIL BUGFIX-1489655 ADD_E
        }
    ...
    protected void starConversation(boolean star) {
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        ...
        if (itemId == R.id.inside_conversation_unread) {
            ...
            handled = true;
        //TS: junwei-xu 2015-09-02 EMAIL BUGFIX-546917 ADD-S
        } else if (itemId == R.id.mark_star) {
            starConversation(true);
            if (mActivity instanceof MailActivity) {
                ((MailActivity) mActivity).invalidateOptionsMenu();
            }
            // TS: zheng.zou 2015-09-19 EMAIL BUGFIX-546917 Add_S
            ConversationUpdater updater = getListController();
            if (updater != null) {
                LogUtils.i(LOG_TAG, "in ACVF.onOptionsItemSelected(), start mark star," +
                        "conversation [ %s, %s ]", mConversation.id, mConversation.starred);
                updater.updateConversation(Conversation.listOf(mConversation), UIProvider.ConversationColumns.STARRED, true);
            }
            // TS: zheng.zou 2015-09-19 EMAIL BUGFIX-546917 Add_E
        } else if (itemId == R.id.mark_unstar) {
            starConversation(false);
            if (mActivity instanceof MailActivity) {
                ((MailActivity) mActivity).invalidateOptionsMenu();
            }
            // TS: zheng.zou 2015-09-19 EMAIL BUGFIX-546917 Add_S
            ConversationUpdater updater = getListController();
            if (updater != null) {
                LogUtils.i(LOG_TAG, "in ACVF.onOptionsItemSelected(), start mark unstar," +
                        "conversation [ %s, %s ]", mConversation.id, mConversation.starred);
                updater.updateConversation(Conversation.listOf(mConversation), UIProvider.ConversationColumns.STARRED, false);
            }
            // TS: zheng.zou 2015-09-19 EMAIL BUGFIX-546917 Add_E
        }
        //TS: junwei-xu 2015-09-02 EMAIL BUGFIX-546917 ADD-E
    ...
    public void onDestroyView() {
        ...
        //TS: jian.xu 2016-01-23 EMAIL BUGFIX-1489655 ADD_S
        if (mContactLoaderCallbacks != null) {
            mContactLoaderCallbacks.unregisterObserver(mContactInfoObserver);
        }
        //TS: jian.xu 2016-01-23 EMAIL BUGFIX-1489655 ADD_E
    }
    ...
    private class MessageLoaderCallbacks
        ...
        public void onLoadFinished(Loader<ObjectCursor<ConversationMessage>> loader,
            ...
            LogUtils.w(LOG_TAG, "AbstractConversationViewFragment              MessageLoaderCallbacks       onLoadFinished      data:"+data);
            // ignore truly duplicate results
            // this can happen when restoring after rotation
            if (mCursor == data) {
                LogUtils.w(LOG_TAG, "AbstractConversationViewFragment              MessageLoaderCallbacks       onLoadFinished      mCursor == data");
                return;
            } else {
                ...
                if (messageCursor.getCount() == 0
                        && (!CursorStatus.isWaitingForResults(messageCursor.getStatus())
                                || mIsDetached)) {
                    LogUtils.w(LOG_TAG, "AbstractConversationViewFragment              MessageLoaderCallbacks       onLoadFinished      mUserVisible:"+mUserVisible);
                    if (mUserVisible) {
                        onError();
                    } else {
                ...
                LogUtils.w(LOG_TAG, "AbstractConversationViewFragment              MessageLoaderCallbacks       onLoadFinished     messageCursor.isLoaded():"+messageCursor.isLoaded());
        ...
        public void onLoaderReset(Loader<ObjectCursor<ConversationMessage>>  loader) {
            LogUtils.w(LOG_TAG, "AbstractConversationViewFragment              MessageLoaderCallbacks       onLoaderReset      ");
    ...
    // we use flag isEasCall to juge if current syncing is eas.
    public void setCallFlag(boolean eas) {
        isEasCall = eas;
    }
