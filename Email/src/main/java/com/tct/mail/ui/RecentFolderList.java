...
import com.tct.mail.utils.LruCache;
...
public final class RecentFolderList {
     ...
     * #loadFromUiProvider(com.tct.mail.content.ObjectCursor)} with the updated
    ...
    /**
     * Update the cache folder from the given cursor.
     */
    public void updateFolderCache(ObjectCursor<Folder> c) {
        if (mAccount == null || c == null) {
            LogUtils.e(TAG, "RecentFolderList.loadFromUiProvider: bad input. mAccount=%s,cursor=%s",
                    mAccount, c);
            return;
        ...
        do {
            final Folder folder = c.getModel();
            if (mFolderCache.containsKey(folder.folderUri.fullUri.toString())) {
                final RecentFolderListEntry entry = new RecentFolderListEntry(folder);
                mFolderCache.putElement(folder.folderUri.fullUri.toString(), entry);
            }
        } while (c.moveToPrevious());
     ...
     */
    public void touchFolder(@NonNull Folder folder, Account account) { // Prevent rare NPE
