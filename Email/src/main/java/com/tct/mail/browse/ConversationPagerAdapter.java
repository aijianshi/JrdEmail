 ...
 *BUGFIX-622635  2015/09/22  jian.xu         [Android L][Email]Discard a draft but it still exist before press back key.
 *BUGFIX-956659  2015/11/20  zheng.zou       [Android L][Email][Monkey][Crash]java.lang.IllegalStateException happened during monkey test
...
import com.tct.mail.ui.ConversationViewFragment;
import com.tct.mail.ui.SecureConversationViewFragment;
import com.tct.mail.utils.FragmentStatePagerAdapter2;
...
public class ConversationPagerAdapter extends FragmentStatePagerAdapter2
    ...
    public void setDetachedMode(boolean detached) {
    ...
    public void notifyDataSetChanged() {
        ...
        if (mController != null && !mDetachedMode && mPager != null) {
            ...
            if (pos == POSITION_NONE && cursor != null && currConversation != null) {
                //TS: jian.xu 2015-09-22 EMAIL BUGFIX-622635 MOD_S
                //Note: show conversation list directly if discard draft from compose activity
                final Folder currentFolder = mController.getFolder();
                final boolean isDraftFolder = currentFolder != null && currentFolder.isDraft();
                if (isDraftFolder) {
                    mController.showConversationList(mController.getCurrentListContext());
                } else {
                ...
                //TS: jian.xu 2015-09-22 EMAIL BUGFIX-622635 MOD_E
            } else {
                ...
                if (frag != null && cursor.moveToPosition(pos) && frag.isUserVisible()) {
                    ...
                    conv.position = pos;
                    LogUtils.i(LOG_TAG, "in CPA.notifyDataSetChanged() conversation [ %s, %s ]", conv.id, conv.starred);
    ...
    //TS: zheng.zou 2015-07-17 EMAIL BUGFIX_-943618 ADD_S
    private boolean isEqualWithCachedUri(Conversation conv1, Conversation conv2) {
        if (conv1 != null && conv2 != null) {
            Uri uri1 = ConversationCursor.uriFromCachingUri(conv1.uri);
            Uri uri2 = ConversationCursor.uriFromCachingUri(conv2.uri);
            return uri1.equals(uri2);
        }

        return false;
    }
    //TS: zheng.zou 2015-07-17 EMAIL BUGFIX_-943618 ADD_E
    ...
    public int getConversationPosition(Conversation conv) {
        ...
        if (isPagingDisabled(cursor)) {
            ...
            if (!conv.equals(def) && !isEqualWithCachedUri(conv, def)) {   //TS: zheng.zou 2015-07-17 EMAIL BUGFIX_-943618 MOD
