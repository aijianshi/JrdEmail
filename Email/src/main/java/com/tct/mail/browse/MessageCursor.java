...
import com.tct.mail.providers.UIProvider.CursorExtraKeys;
...
public class MessageCursor extends ObjectCursor<ConversationMessage> {
    ...
    public int getStateHashCodePop() {
        int hashCode = 17;
        int pos = -1;
        final int stopAt = getCount();
        while (moveToPosition(++pos) && pos < stopAt) {
            hashCode = 31 * hashCode + getMessage().getStateHashCodePop();
        }
        return hashCode;
    }
    ...
    /**
     *
     * @return hashcode without star state
     */
    public int getStateHashCodeWithoutStar(){
        ...
        final int stopAt = getCount() ;
        while (moveToPosition(++pos) && pos < stopAt) {
            hashCode = 31 * hashCode + getMessage().getStateHashCodeWithoutStar();
        }
        return hashCode;
    }
