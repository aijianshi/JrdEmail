...
import com.tct.email.PeekableInputStream;
...
public class ImapResponseParser {
    ...
    private ImapString parseLiteral() throws IOException, MessagingException {
        ...
        if (size > mLiteralKeepInMemoryThreshold) {
            ...
            return new ImapTempFileLiteral(in, mUpdateUIListener);
        } else {
            return new ImapMemoryLiteral(in, mUpdateUIListener);
            // TS: Gantao 2015-12-07 EMAIL BUGFIX_1020377 MOD_E
        }
    }

    // TS: Gantao 2015-12-07 EMAIL BUGFIX_1020377 ADD_S
    //ui update listener to update progress bar
    private Folder.MessageRetrievalListener mUpdateUIListener;

    public void setUpdateUICallback(Folder.MessageRetrievalListener listener) {
        mUpdateUIListener = listener;
    }

    public void removeUpdateUICallback() {
        mUpdateUIListener = null;
    }
