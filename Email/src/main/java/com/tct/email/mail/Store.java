 ...
 *942108         2015/03/19   zhichuan.wei	[SDM][Email] “feature_email_sfrImap_on”
 *942108         2015/03/30   zhichuan.wei	[SDM][Email] “feature_email_sfrImap_on”
 *BUGFIX-1046591 2015/07/23   jin.dong       [stability][crash]Crash in email.
 ===========================================================================
 */
package com.tct.email.mail;
...
import com.tct.email.mail.store.ServiceStore;
...
//[BUGFIX]-Add-BEGIN by TCTNJ.zhichuan.wei,03/19/2015,PR 942108
...
public abstract class Store {
    ...
    //[BUGFIX]-Add-BEGIN by TCTNJ.zhichuan.wei,03/19/2015,PR 942108
    private static final String[] MAILBOX_PROJECTION_FOR_ID = {
        MailboxColumns._ID
    };
    //[BUGFIX]-Add-END by TCTNJ.zhichuan.wei
     ...
     */
    protected static void updateMailbox(Mailbox mailbox, long accountId, String mailboxPath,
                                        char delimiter, boolean selectable, int type, Context context) {
        mailbox.mAccountKey = accountId;
        mailbox.mDelimiter = delimiter;
        String displayPath = mailboxPath;
        int pathIndex = mailboxPath.lastIndexOf(delimiter);
        ...
        mailbox.mType = type;
        //box.mUnreadCount;
        //[BUGFIX]-Del-BEGIN by TCTNB.chen caixia,01/07/2014,PR 579076
        //mailbox.mVisibleLimit = Email.VISIBLE_LIMIT_DEFAULT;
        //[BUGFIX]-Del-END by TCTNB.chen caixia
        if (EmailApplication.isSfrImapFeatureOn() && Account.isSfrImapAccount(context, accountId)) {
            if (type == Mailbox.TYPE_OUTBOX) {
                if (mailboxPath.equals(Mailbox.SFR_IMAP_SENTBOX_SERVER_ID)) {
                    mailbox.mDisplayName = context.getString(R.string.mailbox_name_server_sent);
                    mailbox.mType = Mailbox.TYPE_SENT;
                }
            }else if(type ==Mailbox.TYPE_MAIL){
                //[BUGFIX]-Add-BEGIN by TCTNJ.zhichuan.wei,03/19/2015,PR 942108
                if (mailboxPath.equals(Mailbox.SFR_IMAP_SENTBOX_SERVER_ID)) {
                    mailbox.mDisplayName = context.getString(R.string.mailbox_name_server_sent);
                    mailbox.mType = Mailbox.TYPE_SENT;
                    Cursor cursor = null;
                    try {
                        //[BUGFIX]-Add-BEGIN by TCTNJ.zhichuan.wei,03/30/2015,PR 942108
                        cursor = context.getContentResolver().query(
                                Mailbox.CONTENT_URI,
                                MAILBOX_PROJECTION_FOR_ID,
                                EmailContent.MailboxColumns.SERVER_ID + "=\'Sent\'" + " and "+EmailContent.MailboxColumns.ACCOUNT_KEY +"="+accountId,
                                null,null);
                        //[BUGFIX]-Add-END by TCTNJ.zhichuan.wei
                        if(cursor!=null && cursor.moveToFirst()){
                            final long mailboxId = cursor.getLong(0);
                            AttachmentUtilities.deleteAllMailboxAttachmentFiles(
                                    context, accountId, mailboxId);
                            // Delete the sent box in imap.sfr.fr
                ...
                //[BUGFIX]-Add-END by TCTNJ.zhichuan.wei
                if (mailboxPath.equals(Mailbox.SFR_IMAP_DRAFTBOX_SERVER_ID)) {
                    mailbox.mDisplayName = context.getString(R.string.mailbox_name_display_draft);
                }
            }
        }
    }
