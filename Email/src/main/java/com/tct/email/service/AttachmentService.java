 ...
 *BUGFIX-928641  2015-02-11   gengkexue     [Stability][Email] Power Consumption is high during test Email
 ...
 *BUGFIX-967385  2015/11/23   kaifeng.lu     [Orange Spain] Exchange email sync stops very often
 ===========================================================================
 */
...
import com.tct.email.AttachmentInfo;
import com.tct.email.EmailConnectivityManager;
...
public class AttachmentService extends Service implements Runnable {
    ...
    private static final int MAX_INLINE_ATTACHMENTS_TO_CHECK = 50;
    private static final String EXTRA_ATTACHMENT_ID =
            "com.tct.email.AttachmentService.attachment_id";
    private static final String EXTRA_ATTACHMENT_FLAGS =
            "com.tct.email.AttachmentService.attachment_flags";
            ...
            if (req != null) {
                ...
                switch (statusCode) {
                    ...
                    default:
                        ...
                        LogUtils.d(LOG_TAG, "Attachment #%d is done, Attachment statusCode is #%d", attachmentId, statusCode);
    ...
    public static void attachmentChanged(final Context context, final long id, final int flags) {
        ...
        //NOTE: God way here that do not download the attachment if the message is partial,user
        //can trigger the download after click the downloadRemain btn.@..@
        //Only for EAS account.
        boolean messagePartial = false;
        boolean downloadAll = false;
        boolean isEas = false;
        if ((id >= 0) && (flags >= 0)) {
            Attachment att = Attachment.restoreAttachmentWithId(context, id);
            if (att != null) {
                Message msg = Message.restoreMessageWithId(context, att.mMessageKey);
                Account account = Account.restoreAccountWithId(context, att.mAccountKey);
                String protol = Account.getProtocol(context, att.mAccountKey);
                if (msg != null) {
                    messagePartial = msg.mFlagLoaded == Message.FLAG_LOADED_PARTIAL;
                }
                if (account != null) {
                    downloadAll = account.getDownloadOptions() == Utility.ENTIRE_MAIL;
                }
                if (protol != null) {
                    isEas = TextUtils.equals(protol, HostAuth.SCHEME_EAS);
                }
            }
        }
        // When account set to HeaderOnly,and current account is eas,message is partial, DO not
        // start AMS to download the attachment.
        if (isEas && !downloadAll && messagePartial) {
            return;
        }
    ...
    public synchronized void onChange(final Context context, final Attachment att) {
        ...
        if (priority == PRIORITY_NONE) {
            ...
            if (req == null) {
                ...
                if (!attachInfo.isEligibleForDownload() && !attachInfo.mAllowInstall) {
    ...
    void markAttachmentAsFailed(final Attachment att) {
        ...
        // TS: kaifeng.lu  2015-11-23 Email BUGFIX_967385  MOD_S
        att.mFlags &= ~flags;
        att.mFlags |= Attachment.FLAG_DOWNLOAD_FAILED;
        cv.put(AttachmentColumns.FLAGS, att.mFlags);
        // TS: kaifeng.lu  2015-11-23 Email BUGFIX_967385  MOD_E
    ...
    void markAttachmentAsCompleted(final Attachment att) {
        final ContentValues cv = new ContentValues();
        // TS: kaifeng.lu  2015-11-23 Email BUGFIX_967385  MOD_S
        final int flags = Attachment.FLAG_DOWNLOAD_FORWARD | Attachment.FLAG_DOWNLOAD_USER_REQUEST| Attachment.FLAG_DOWNLOAD_FAILED;
        // TS: kaifeng.lu  2015-11-23 Email BUGFIX_967385  MOD_E
    ...
    /**
     * Set the ui state to mark this download is started
     * @param att The attachment that was started to download
     */
    void markAttachmentAsDownloading(final Attachment att) {
        final ContentValues cv = new ContentValues();
        cv.put(AttachmentColumns.UI_STATE, AttachmentState.DOWNLOADING);
        att.update(this, cv);
    }

    /**
     * Set the ui state to mark the download is on the queue waiting to download
     * @param att
     */
    void markAttachmentAsReadyDownloading(final Attachment att){
        final ContentValues cv = new ContentValues();
        cv.put(AttachmentColumns.UI_STATE, AttachmentState.READYDOWNLOADING);
        att.update(this, cv);
    }
    ...
    synchronized void processQueue() {
        ...
        while (change != null) {
            ...
            if (attachment == null) {
                ...
                // AM: Kexue.Geng 2015-02-11 EMAIL BUGFIX_928641 MOD_S
                change = sAttachmentChangedQueue.poll();
                // AM: Kexue.Geng 2015-02-11 EMAIL BUGFIX_928641 MOD_E
                continue;
            }
        ...
        while (mDownloadsInProgress.size() < MAX_SIMULTANEOUS_DOWNLOADS) {
            ...
            // TS: tao.gan 2016-01-26 EMAIL BUGFIX-1483135 DEL_S
            //I think it's not necessary to enforce per-account limit.
...
//                continue;
//            }
            // TS: tao.gan 2016-01-26 EMAIL BUGFIX-1483135 DEL_E
        ...
        //In any case,we need to automatically download the inline attachment for feature
        //"Always show pictures",not care it's WIFI or data.
        final Uri lookupInlineUri = EmailContent.uriWithLimit(Attachment.CONTENT_URI,
                MAX_INLINE_ATTACHMENTS_TO_CHECK);
        final Cursor inlineCursor = this.getContentResolver().query(lookupInlineUri,
                Attachment.CONTENT_PROJECTION,
                // TS: xinlei.sheng 2016-3-3 EMAIL BUGFIX_1664255 ADD_S
                EmailContent.Attachment.PRECACHE_SELECTION_NOT_DOWNLOADED_INLINE,
                // TS: xinlei.sheng 2016-3-3 EMAIL BUGFIX_1664255 ADD_E
                null, AttachmentColumns._ID + " DESC");
        try {
            while (inlineCursor.moveToNext()) {
                final Attachment att = new Attachment();
                att.restore(inlineCursor);
                ...
                if (tryCount != null && tryCount > MAX_DOWNLOAD_RETRIES) {
                    ...
                    LogUtils.w(LOG_TAG,
                            "Too many failed attempts for inline attachment #%d ", att.mId);
                    continue;
                }
                final DownloadRequest req = new DownloadRequest(this, att);
                LogUtils.d(LOG_TAG,"Start automatically download the inline attachment #%d ", att.mId);
                tryStartDownload(req);
                // TS: zheng.zou 2016-3-30 EMAIL BUGFIX_1841298 DEL_S
                //Note: we should trigger all inline attachment download, instead of only one.
//                break;
                // TS: zheng.zou 2016-3-30 EMAIL BUGFIX_1841298 DEL_E
            }
        } finally {
            inlineCursor.close();
        }
        ...
        //Here we just auto download not inline attachments.
        final Cursor c = this.getContentResolver().query(lookupUri,
                Attachment.CONTENT_PROJECTION,
                EmailContent.Attachment.PRECACHE_INBOX_SELECTION_NOT_INLINE,
                null, AttachmentColumns._ID + " DESC");
            ...
            while (c.moveToNext()) {
                ...
                if (account == null) {
                    ...
                    if (info.isEligibleForDownload()) {
                        ...
                        if (canPrefetchForAccount(account, cacheDir)) {// TS: tao.gan 2015-08-12 EMAIL FEATURE-ID MOD
                            final Integer tryCount = mAttachmentFailureMap.get(att.mId);
                            if (tryCount != null && tryCount > MAX_DOWNLOAD_RETRIES) {
                                // move onto the next attachment
                                LogUtils.w(LOG_TAG,
    ...
    private void startDownload(final EmailServiceProxy service, final DownloadRequest req)
        ...
        // TS: Gantao 2015-12-24 EMAIL BUGFIX-1175829 ADD_S
        //Mark the attachment is started to download.
        final Attachment attachment =
                Attachment.restoreAttachmentWithId(AttachmentService.this, req.mAttachmentId);
        if(attachment != null) {
            LogUtils.d(LOG_TAG, "Starting download for Attachment #%d, mark as downloading", req.mAttachmentId);
            markAttachmentAsDownloading(attachment);
        }
        // TS: Gantao 2015-12-24 EMAIL BUGFIX-1175829 ADD_E
    ...
    synchronized void cancelDownload(final DownloadRequest req) {
        ...
        if (req.mRetryCount > CONNECTION_ERROR_MAX_RETRIES) {
            ...
            if (att != null) {
                markAttachmentAsFailed(att);
                LogUtils.w(LOG_TAG, "cancel att download, mark as failed AttID = " + req.mAttachmentId);
            }
            // TS: kaifeng.lu  2015-11-23 Email BUGFIX_967385  MOD_E
        } else {
            debugTrace("Moving to end of queue, will retry #%d", req.mAttachmentId);
            // TS: Gantao 2015-12-24 EMAIL BUGFIX-1175829 ADD_S
            //Mark the attachment is on the queue wating to download.
            if(att != null) {
                LogUtils.d(LOG_TAG, "retry do download #%d, mark as ready", req.mAttachmentId);
                markAttachmentAsReadyDownloading(att);
            }
            // TS: Gantao 2015-12-24 EMAIL BUGFIX-1175829 ADD_E
    ...
    synchronized void endDownload(final long attachmentId, final int statusCode) {
        ...
        if (statusCode == EmailServiceStatus.CONNECTION_ERROR) {
            ...
            if (req != null) {
                ...
                if (req.mRetryCount > CONNECTION_ERROR_MAX_RETRIES) {
                    ...
                    if (att != null) {
                        markAttachmentAsFailed(att);
                        LogUtils.w(LOG_TAG, "end download, att download failed, mark as failed AttID = " + req.mAttachmentId);
                    }
                    // TS: kaifeng.lu  2015-11-23 Email BUGFIX_967385  MOD_E
        ...
        if (attachment != null) {
            ...
            } else if (statusCode == EmailServiceStatus.IO_ERROR){
                LogUtils.d(LOG_TAG, "Attachment #%d download failed!", attachment.mId);
