...
/*           |                      |                      |plemented         */
/* ----------|----------------------|----------------------|----------------- */
/* 10/24/2014|zhujian.shao          |736411                |[HOMO][HOMO][Oran */
/*           |                      |                      |ge][Homologation] */
/*           |                      |                      | Exchange Active  */
/*           |                      |                      |Sync Priority     */
/* ----------|----------------------|----------------------|----------------- */
 ...
 *BUGFIX-984331  2015/4/22    gengkexue       [Android5.0][Email][REG][FC]Email will force close when we slide to delete an email
 ...
 *BUGFIX-988783  2015/11/30   junwei-xu       Use rel3 email apk to overide fsr email apk crash
 ===========================================================================  */
/******************************************************************************/
...
import com.tct.emailcommon.service.LegacyPolicySet;
...
import com.tct.emailcommon.provider.EmailContent.AccountInfoColumns;
...
public final class DBHelper {
    ...
    //              version above 12.0
    // Version 128: Add download options
    // Version 129: Exchange Active Sync Priority
    //[FEATURE]-Add-BEGIN by TSNJ zhujian.shao,10/24/2014,FR 736411 (porting from FR622697)
    //public static final int DATABASE_VERSION = 129;
    //[FEATURE]-Add-END by TSNJ zhujian.shao

    // Version 130: Add AccountInfo table
    // Version 131: Add RealUri column to Attachment Table
    // Version 132: Add dirty flag field to message (for Bad sync key recovery)
    // Version 133: Add dirty flag field to Message_Updates and Message_Deletes (for Bad sync key recovery)
    // Version 134: Add inline images field to Account table (for mark whether always show inline images)
    //[FEATURE]-Add-BEGIN by TSNJ.qinglian.zhang,10/28/2014,FR 736417
    //TS: zhonghua.tuo 2015-3-3 EMAIL BUGFIX_936728 MOD_S
    // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_S
    // Version 135:Add mail send status to message table(Exchange Email resend mechanism)
    // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_E
    // Version 136: Add Recipient table
    // Version 137: Add isInline to attachment table(for feature always show picture)
    // Version 138: Add syncCalendarLookback column to Account table
    // Version 139: Add disallow simple password to policy table
    // Version 140: Add replyTo column to account table
    // Version 141: Add messageSize column to message table
    ...
    //[FEATURE]-Add-BEGIN by TCTNB.fu.zhang,04/08/2014,622697
    //exchange active sync priority feature
    static void createMessageTable(Context context, SQLiteDatabase db) {
        String messageColumns = MessageColumns.DISPLAY_NAME + " text, "
            ...
            + MessageColumns.FLAG_PRIORITY + " integer, "
            ...
            // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_S
            + MessageColumns.SENDING_STATUS + " integer, "
            // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_E
            + MessageColumns.MESSAGE_SIZE + " integer "    //TS: zheng.zou 2016-1-14 EMAIL TASK_1431225 ADD
            + ");";
    ...
    static void createAccountTable(SQLiteDatabase db) {
        ...
        String s = " (" + AccountColumns._ID + " integer primary key autoincrement, "
            ...
            + AccountColumns.DOWNLOAD_OPTIONS + " integer, "
            + AccountColumns.INLINE_IMAGES + " integer, "// TS: tao.gan 2015-08-12 EMAIL FEATURE-ID ADD
            + AccountColumns.SYNC_CALENDAR_LOOKBACK + " integer, "
            + AccountColumns.REPLY_TO + " text" // TS: tao.gan 2015-12-25 EMAIL FEATURE-1239148 ADD
            + ");";
        //[FEATURE]-Add-END by TSCD.Chao Zhang
        //TS: junwei-xu 2015-09-29 EMAIL FEATURE-1093309 MOD_E
        db.execSQL("create table " + Account.TABLE_NAME + s);
        // Deleting an account deletes associated Mailboxes, Recipients and HostAuth's
    ...
    static void createPolicyTable(SQLiteDatabase db) {
        String s = " (" + PolicyColumns._ID + " integer primary key autoincrement, "
            ...
            + PolicyColumns.PROTOCOL_POLICIES_UNSUPPORTED + " text, "
                ...
                + PolicyColumns.DISALLOW_SIMPLE_DEVICE_PASSWORD + " integer"
    ...
    static void createAttachmentTable(SQLiteDatabase db) {
        String s = " (" + AttachmentColumns._ID + " integer primary key autoincrement, "
            ...
            + AttachmentColumns.REAL_URI + " text, "
            //TS: zhonghua.tuo 2015-3-3 EMAIL BUGFIX_936728 MOD_E
            + AttachmentColumns.ISINLINE + " interger"
            + ");";
    ...
    //[FEATURE]-Add-BEGIN by TSNJ.qinglian.zhang,10/28/2014,FR 736417
    static void createAccountInfoTable(SQLiteDatabase db) {
        String s = " (" + EmailContent.RECORD_ID
                + " integer primary key autoincrement, "
                + AccountInfoColumns.ACCOUNT_ID + " integer, "
                + AccountInfoColumns.IS_USEPROXY + " integer, "
                + AccountInfoColumns.PROXY_ADDRESS + " text, "
                + AccountInfoColumns.PROXY_PORT + " integer, "
                + AccountInfoColumns.PROXY_USERNAME + " text, "
                + AccountInfoColumns.PROXY_USERPASS + " text " + ");";
        db.execSQL("create table " + AccountInfo.TABLE_NAME + s);
    }
    //[FEATURE]-Add-END by TSNJ.qinglian.zhang

    //TS: junwei-xu 2015-09-17 EMAIL BUGFIX-569939 ADD-S
    static void createRecipientTable(SQLiteDatabase db) {
        String s = " (" + EmailContent.RECORD_ID
                + " integer primary key autoincrement, "
                + EmailContent.RecipientColumns.EMAIL_ADDRESS + " text, "
                + EmailContent.RecipientColumns.DISPLAY_NAME + " text, "
                + EmailContent.RecipientColumns.ACCOUNT_KEY + " integer, unique ("
                + EmailContent.RecipientColumns.EMAIL_ADDRESS + ", "
                + EmailContent.RecipientColumns.DISPLAY_NAME + "));";
        db.execSQL("create table " + EmailContent.Recipient.TABLE_NAME + s);
    }
    ...
    protected static class DatabaseHelper extends SQLiteOpenHelper {
        ...
        public void onUpgrade(SQLiteDatabase db, int oldVersion, int newVersion) {
            ...
            if (oldVersion <= 127) {
                try {
                     db.execSQL("alter table " + Account.TABLE_NAME
                             + " add column " + AccountColumns.DOWNLOAD_OPTIONS + " integer;");
                 } catch (SQLException e) {
                     // Shouldn't be needed unless we're debugging and interrupt the process
                     LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 123 to 124 " + e);
                 }
            }
            //[FEATURE]-Add-END by TSCD.Chao Zhang
            ...
            if (oldVersion <=128) {
                 try {
                      db.execSQL("alter table " + Message.TABLE_NAME
                              + " add column " + MessageColumns.FLAG_PRIORITY + " integer;");
                      db.execSQL("alter table " + Message.DELETED_TABLE_NAME
                              + " add column " + MessageColumns.FLAG_PRIORITY + " integer;");
                      db.execSQL("alter table " + Message.UPDATED_TABLE_NAME
                              + " add column " + MessageColumns.FLAG_PRIORITY + " integer;");
            ...
            if(oldVersion<=130){
                try {
                    db.execSQL("alter table " + Attachment.TABLE_NAME
                            + " add column " + AttachmentColumns.REAL_URI + " text");
                } catch (Exception e) {
                    // TODO: handle exception
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 133 to 134 " + e);
                }
            }
            ...
            if (oldVersion <= 131) {
                try {
                    ...
                    /// M: May reach here if upgrade from MTK JB2 Email because "dirty" is
                     // existed already
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 131 to 132 ", e);
                }
            }
            // AM: Kexue.Geng 2015-04-17 EMAIL BUGFIX_969850 MOD_E
            // AM: Kexue.Geng 2015-04-22 EMAIL BUGFIX_984331 MOD_S
            if (oldVersion <= 132) {
                try {
                    db.execSQL("alter table " + Message.DELETED_TABLE_NAME
                            + " add column " + MessageColumns.DIRTY + " text;");
                    ...
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 132 to 133 ", e);
                }
            }
            // AM: Kexue.Geng 2015-04-22 EMAIL BUGFIX_984331 MOD_E
            // TS: tao.gan 2015-08-12 EMAIL FEATURE-ID ADD_S
            if (oldVersion <= 133) {
                try {
                    db.execSQL("alter table " + Account.TABLE_NAME
                            + " add column " + AccountColumns.INLINE_IMAGES + " integer;");
                } catch (SQLException e) {
                    // Shouldn't be needed unless we're debugging and interrupt the process
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 133 to 134 " + e);
                }
            ...
            if (oldVersion <= 134) {
                try {
                    db.execSQL("alter table " + Message.TABLE_NAME
                            + " add column " + MessageColumns.SENDING_STATUS + " integer;");
                    db.execSQL("alter table " + Message.DELETED_TABLE_NAME
                            + " add column " + MessageColumns.SENDING_STATUS + " integer;");
                    db.execSQL("alter table " + Message.UPDATED_TABLE_NAME
                            + " add column " + MessageColumns.SENDING_STATUS + " integer;");
                } catch (SQLException e) {
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 134 to 135 ", e);
                }
            }
            // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_E
            //TS: junwei-xu 2015-09-17 EMAIL BUGFIX-569939 ADD-S
            ...
            if(oldVersion<=136){
                try {
                    db.execSQL("alter table " + Attachment.TABLE_NAME
                            + " add column " + AttachmentColumns.ISINLINE + " integer");
                } catch (Exception e) {
                    // TODO: handle exception
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 133 to 134 " + e);
                }
            ...
            if (oldVersion <= 137) {
                try {
                    db.execSQL("alter table " + Account.TABLE_NAME
                            + " add column " + AccountColumns.SYNC_CALENDAR_LOOKBACK + " integer;");
                } catch (SQLException e) {
                    // Shouldn't be needed unless we're debugging and interrupt the process
                    LogUtils.w(TAG, "Exception upgrading EmailProvider.db from 134 to 135 " + e);
                }
            ...
            //TS: junwei-xu 2015-11-30 EMAIL BUGFIX-988783 ADD_S
            //update db again to ensure db field completeness, the apk will replace apk on other branch,
            //which has different db version 136, and some field may not be updated when upgrade apk based on that.
            //So we do db field again to avoid that.
            if (oldVersion <= 137) {
                try {
                    db.execSQL("alter table " + Account.TABLE_NAME
                                + " add column " + AccountColumns.INLINE_IMAGES + " integer;");
                    ...
                    // Shouldn't be needed unless we're debugging and interrupt the process
                    LogUtils.w(TAG, "Exception when add INLINE_IMAGES" + e);
                }

                try {
                    createRecipientTable(db);
                } catch (SQLException e) {
                    LogUtils.w(TAG, "Exception when createRecipientTable" + e);
                }
                ...
                try {
                    ...
                    LogUtils.w(TAG, "Exception when add ISINLINE" + e);
                }
            }
            //TS: junwei-xu 2015-11-30 EMAIL BUGFIX-988783 ADD_E
            ...
            if (oldVersion <= 138) {
                try {
                    db.execSQL("alter table " + Policy.TABLE_NAME
                            + " add column "
                            + PolicyColumns.DISALLOW_SIMPLE_DEVICE_PASSWORD
                            + " integer;");
                } catch (SQLException e) {
                    // Shouldn't be needed unless we're debugging and interrupt
                    // the process
                    LogUtils.w(TAG,"Exception upgrading EmailProvider.db from 138 to 139 ",e);
                }
            }
            // TS: tao.gan 2015-09-22 EMAIL BUGFIX-1068066 ADD_E
            ...
            if (oldVersion <= 139) {
                try {
                    db.execSQL("alter table " + Account.TABLE_NAME
                            + " add column "
                            + AccountColumns.REPLY_TO
                            + " text;");
                } catch (SQLException e) {
                    // Shouldn't be needed unless we're debugging and interrupt
                    // the process
                    LogUtils.w(TAG,"Exception upgrading EmailProvider.db from 139 to 140 ",e);
                }
            }
            ...
            if (oldVersion <= 140) {
                try {
                    db.execSQL("alter table " + Message.TABLE_NAME
                            + " add column " + MessageColumns.MESSAGE_SIZE + " integer;");
                    db.execSQL("alter table " + Message.DELETED_TABLE_NAME
                            + " add column " + MessageColumns.MESSAGE_SIZE + " integer;");
                    db.execSQL("alter table " + Message.UPDATED_TABLE_NAME
                            + " add column " + MessageColumns.MESSAGE_SIZE + " integer;");
                    ...
                    // Shouldn't be needed unless we're debugging and interrupt
                    // the process
                    LogUtils.w(TAG,"Exception upgrading EmailProvider.db from 140 to 141 ",e);
                }
            }
