 ...
 *======================================================================================================
 *BUGFIX-930472  2015-02-17   gengkexue       [stability]Email occure error in 2# stability mobilephone.
 *======================================================================================================
 */

package com.tct.emailsync;
...
import com.tct.emailcommon.service.IEmailServiceCallback.Stub;
...
public abstract class SyncManager extends Service implements Runnable {
    ...
    /**
     * Returns the AccountManager type (e.g. com.tct.exchange) for this sync service
     */
                ...
                synchronized (mAccountList) {
                    for (Account account : mAccountList) {
                        ...
                        if (!currentAccounts.contains(account.mId) && !accountIncomplete) {
	                        ...
	                        releaseAccountMailboxes(account.mId);//[BUGFIX]-ADD-By TCTNB.wen.zhuang,03/07/2014 PR-611449
    ...
    static public synchronized EmailClientConnectionManager getClientConnectionManager(
        ...
        if (mgr == null) {
            ...
            // AM: Kexue.Geng 2015-02-17 EMAIL BUGFIX_930472 MOD_S
            if (port <= 0) {
                port = ssl ? 443 : 80;
            }
            // AM: Kexue.Geng 2015-02-17 EMAIL BUGFIX_930472 MOD_E
    ...
    private void acquireWakeLock(long id) {
        synchronized (mWakeLocks) {
            checkConnectivity();  //[FEATURE]-ADD-By
                                  //CDTS.zhonghua.tuo,04/14/2014
                                  //FR-631898,
    ...
    //[FEATURE]-Add-BEGIN by CDTS.zhonghua.tuo,04/14/2014,FR-631898,
    private void checkConnectivity() {
        ConnectivityManager cm =
            (ConnectivityManager)SyncManager.this.getSystemService(Context.CONNECTIVITY_SERVICE);
        NetworkInfo info = cm.getActiveNetworkInfo();
        if (info != null && info.isConnected()) {
            return;
        }
        doForceReleaseWakeLock();
    }
    ...
    private void doForceReleaseWakeLock() {
        mWakeLocks.clear();
        if (mWakeLock != null) {
            mWakeLock.release();
            log("force release weak lock!");
        }
        mWakeLock = null;
    }
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (b != null) {
                ...
                if (state == State.CONNECTED) {
                    ...
                    info += " DISCONNECTED";
                    log(info);
                    forceReleaseWakeLock();  //[FEATURE]-ADD-By
                                             //TSCD.xiao.jian,04/17/2014
                                             //FR-634941
                    kick("disconnected");
                }
            }
    ...
    private void waitForConnectivity() {
        ...
        while (!sStop) {
            NetworkInfo info = cm.getActiveNetworkInfo();
            if (info != null && info.isConnected()) {  //[FEATURE]-ADD-By
                                                       //CDTS.zhonghua.tuo,04/14/2014
                                                       //FR-631898,
                mNetworkInfo = info;
     ...
     * com.tct.email) and hasn't been restarted. See the comment for onCreate for details
     */
            ...
            while (c.moveToNext()) {
                ...
                if (service == null) {
                    ...
                    if (syncInterval == Mailbox.CHECK_INTERVAL_PUSH) {
                        Mailbox m = EmailContent.getContent(getContext(),c, Mailbox.class);
    ...
    //[BUGFIX]-Add-BEGIN by TCTNB.wen.zhuang,03/07/2014,PR-611449,release wakelock
    /*===========================================================================
     *
     * Name            releaseAccountMailboxes

     * Author          Xu Danbin
     *
     * Date            2013/01/09
     *
     * Description     release all wake lock for gived account
     *
     * Iputs           acctId: account ID
     *
     * Outputs         none
     *
     *
     *===========================================================================*/
    private void releaseAccountMailboxes(Long acctId) {
        // check if account's mailbox wakelock is released
        synchronized (mWakeLocks) {
            //TctLog.i(TAG, "releaseAccountMailboxes mWakeLocks:"
            //            + mWakeLocks.toString() + " acctId:" + acctId);
            if (INSTANCE != null && mWakeLock != null) {
                //TctLog.i(TAG, "releaseAccountMailboxes mWakeLock:" + mWakeLock);
                List<Long> delList = new ArrayList<Long>();
                for (Long mid : mWakeLocks.keySet()) {
                    if (mid == EXTRA_MAILBOX_ID) continue;
                    Mailbox box = Mailbox.restoreMailboxWithId(INSTANCE, mid);
                    if (box == null) {
                        // box is null maybe account is deleted already
                        //mWakeLocks.remove(mid);
                        delList.add(mid);
                        //TctLog.i(TAG, "releaseAccountMailboxes remove null account box id: "
                        //        + mid);
                    } else {
                        Account acct = Account.restoreAccountWithId(INSTANCE,
                                box.mAccountKey);
                        if (acct.mId == acctId) {
                            //mWakeLocks.remove(mid);
                            delList.add(mid);
                            //TctLog.i(TAG, "releaseAccountMailboxes remove box id: " + mid
                            //        + " " + box.mDisplayName + " for account: "
                            //        + acct.mDisplayName);
                        }
                    }
                }
                for (Long mailboxId : delList) {
                    mWakeLocks.remove(mailboxId);
                }
                if (mWakeLocks.isEmpty()) {
                    if (mWakeLock != null) {
                        mWakeLock.release();
                        //TctLog.i(TAG, "releaseAccountMailboxes WakeLock RELEASED");
                    }
                    mWakeLock = null;
                }
            }
    ...
    //[BUGFIX]-Add-END by TCTNB.wen.zhuang
}
