...
*BUGFIX-1552367 2016-02-04   zheng.zou      [Android 6.0][Email]The recipient head icon is missed after rotate phone to landscape mode.
...
import com.tct.fw.ex.chips.BaseRecipientAdapter.DirectoryListQuery;
import com.tct.fw.ex.chips.BaseRecipientAdapter.DirectorySearchParams;
import com.tct.fw.ex.chips.DropdownChipLayouter.AdapterType;
import com.tct.fw.ex.chips.Queries.Query;
...
public class RecipientAlternatesAdapter extends CursorAdapter {
    ...
    public static void getMatchingRecipients(Context context, BaseRecipientAdapter adapter,
        ...
        getMatchingRecipientsFromDirectoryQueries(context, recipientEntries,
                keys, account, matchesNotFound, query, callback);
            ...
            if (paramsList != null) {
                ...
                for (String unresolvedAddress : unresolvedAddresses) {
                    ...
                    for (int i = 0; i < paramsList.size(); i++) {
                        try {
                            //TS: junwei-xu 2016-02-02 EMAIL BUGFIX-1469249 MOD_S
                            //Note: the key is "name+address", we should tokenize the address to query.
                            Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(unresolvedAddress);
                            String address = tokens.length > 0 ? tokens[0].getAddress() : unresolvedAddress;
                            directoryContactsCursor = doQuery(address, 1,
                                    paramsList.get(i).directoryId, account,
                                    context.getContentResolver(), query);
                            //TS: junwei-xu 2016-02-02 EMAIL BUGFIX-1469249 MOD_E
                        } finally {
    ...
    private static HashMap<String, RecipientEntry> processContactEntries(Cursor c,
        ...
        if (c != null && c.moveToFirst()) {
            //TS: jian.xu 2016-01-31 EMAIL BUGFIX-1527821 MOD-S
            //Note: construct "name + address" as hash map's key.
            String name, address;
            Rfc822Token person;
            do {
                address = c.getString(Queries.Query.DESTINATION);
                //TS: rong-tang 2016-03-28 EMAIL BUGFIX-1863457 MOD_S
                name = Rfc822Validator.fixInvalidName(c.getString(Queries.Query.NAME));
                //TS: rong-tang 2016-03-28 EMAIL BUGFIX-1863457 MOD_E
                person = new Rfc822Token(name, address, null);
                ...
                final RecipientEntry recipientEntry =
                        getBetterRecipient(recipientEntries.get(generateKey(person)), newRecipientEntry);

                recipientEntries.put(generateKey(person), recipientEntry);
                if (Log.isLoggable(TAG, Log.DEBUG)) {
    ...
    //TS: junwei-xu 2016-02-02 EMAIL BUGFIX-1469249 ADD_S
    /**
     * Generate the key for RecipientMatchCallback's hash map key,
     * the key should keep in line with RecipientReplacementTask's key */
    private static String generateKey(Rfc822Token token) {
        //TS: zheng.zou 2016-02-04 EMAIL BUGFIX-1552367 MOD_S
        //Note: For some contact which name contains illeage character,
        //we should set name as null, must keep in line with RecipientReplacementTask's key.
        if (!Rfc822Validator.isNameValid(token.getName()) ||
                TextUtils.equals(token.getName(), token.getAddress())) {
            token.setName(null);
        }
        //TS: zheng.zou 2016-02-04 EMAIL BUGFIX-1552367 MOD_E
        return token.toString() + ", ";
    }
    //TS: junwei-xu 2016-02-02 EMAIL BUGFIX-1469249 ADD_E
    ...
    public RecipientAlternatesAdapter(Context context, long contactId, Long directoryId,
        ...
        this(context, contactId, directoryId, lookupKey, currentId, queryMode, listener,
                dropdownChipLayouter, null,"");// TS: chenyanhua 2014-02-03 EMAIL BUGFIX_894492 MOD
    }

    public RecipientAlternatesAdapter(Context context, long contactId, Long directoryId,
            String lookupKey, long currentId, int queryMode, OnCheckedItemChangedListener listener,
            DropdownChipLayouter dropdownChipLayouter, StateListDrawable deleteDrawable,String currentAddress) {// TS: chenyanhua 2014-02-03 EMAIL BUGFIX_894492 MOD
        super(context,
                getCursorForConstruction(context, contactId, directoryId, lookupKey, queryMode,currentAddress), 0);// TS: chenyanhua 2014-02-03 EMAIL BUGFIX_894492 MOD
    ...
    private static Cursor getCursorForConstruction(Context context, long contactId,
            Long directoryId, String lookupKey, int queryType,String currentAddress) {
        final Cursor cursor;
         ...
         // TS: chenyanhua 2015-02-03 EMAIL BUGFIX-894492 ADD_E
            ...
            cursor = context.getContentResolver().query(
                    ...
                    selection.toString(), new String[] {
                        String.valueOf(contactId),currentAddress
                    }, null);// TS: chenyanhua 2014-02-03 EMAIL BUGFIX_894492 MOD
        } else {
            final Uri uri;
            final StringBuilder selection = new StringBuilder();
