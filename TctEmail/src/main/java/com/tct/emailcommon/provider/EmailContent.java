...
public abstract class EmailContent {
    ...
    public static final int LAST_SYNC_RESULT_SECURITY_ERROR =
            UIProvider.UIPROVIDER_LASTSYNCRESULTAUTH_SECURITY_ERROR;
    public static final int LAST_SYNC_RESULT_CONNECTION_ERROR =
            UIProvider.UIPROVIDER_LASTSYNCRESULTAUTH_CONNECTION_ERROR;
    public static final int LAST_SYNC_RESULT_INTERNAL_ERROR =
            UIProvider.UIPROVIDER_LASTSYNCRESULTAUTH_INTERNAL_ERROR;
    ...
    public static synchronized void init(Context context) {
        if (AUTHORITY == null) {
            ...
            MAILBOX_MOST_RECENT_UNREAD_MESSAGE_URI = Uri.parse("content://" + AUTHORITY +
                    "/mailboxMostRecentUnread");                //TS:zheng.zou 2015-12-17 EMAIL BUGFIX_861247  ADD
    ...
    public static final class Body extends EmailContent {
        ...
        public static long restoreBodySourceKey(Context context, long messageId) {
            return Utility.getFirstRowLong(context, Body.CONTENT_URI,
                    ...
                    1, 0L);       // TS: zheng.zou 2015-04-04 EMAIL BUGFIX-964415 MOD
    ...
    public static final class Message extends EmailContent {
        ...
        // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_S
        public static final String[] ID_COLUMN_WITH_STATUS_PROJECTION = {
                MessageColumns._ID, MessageColumns.SENDING_STATUS
        };
        // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_E
        ...
        public static final String FLAG_LOADED_COMPLETE_SELECTION =
                MessageColumns.FLAG_LOADED + " = " + Message.FLAG_LOADED_COMPLETE;
        ...
        public static final String ALL_INBOX_COMPLETE_SELECTION =
                MessageColumns.MAILBOX_KEY + " IN ("
                        + "SELECT " + MailboxColumns._ID + " FROM " + Mailbox.TABLE_NAME
                        + " WHERE " + MailboxColumns.TYPE + " = " + Mailbox.TYPE_INBOX
                        + ")"
                        + " AND " + FLAG_LOADED_COMPLETE_SELECTION;
        // TS: tao.gan 2015-08-12 EMAIL FEATURE-ID ADD_E
        /**
        ...
        public static final String SORT_ORDER_BY_PRIORITY =
                MessageColumns.FLAG_PRIORITY;
        ...
        // TS: chao.zhang 2015-09-14 EMAIL FEATURE-585337 ADD_E
        public static String buildLocalSearchSelection(
                Context context, long accountId, long mailboxId, String query, int localSearchField) {
            ...
            if (localSearchField == UIProvider.UIPROVIDER_LOCAL_SEARCH_ALL) {
                String messageIdSelection = buildMessageIdSelection(context, accountId, mailboxId, query, localSearchField);
                selection.append(" AND (").append(MessageColumns.SUBJECT).append(" LIKE '%").append(query).append("%' OR ")
                        .append(messageIdSelection).append(")");
                ...
                selection.append(" AND ").append(MessageColumns.SUBJECT).append(" LIKE '%").append(query).append("%'");
            }
            return selection.toString();
        }

        public static String buildMessageIdSelection(Context context,
                                                     long accountId, long mailboxId, String query, int localSearchField) {
            ...
            try {
                c = context.getContentResolver().query(Message.CONTENT_URI,
                        projection, messageListSelection, null, null);
                while (c.moveToNext()) {
         ...
         */
        private static Set<String> matchBodyQueryFromText(Context context, long[] messageIds, String
                [] queryTermsForBody) {
            LogUtils.logFeature(
                    LogTag.SEARCH_TAG,
...
ng[]{
    ...
    public static final class Attachment extends EmailContent implements Parcelable {
        ...
        public static final String ATTACHMENT_PROVIDER_LEGACY_URI_PREFIX =
                "content://com.tct.email.attachmentprovider";
     ...
     // TS: Gantao 2015-09-19 EMAIL BUGFIX_570084 ADD_E
        ...
        public static final String PRECACHE_SELECTION_NOT_INLINE =
                AttachmentColumns.CONTENT_URI + " isnull AND " + AttachmentColumns.FLAGS + "=0"
                + " AND " + AttachmentColumns.CONTENT_ID + " isnull";
        // Attachments with an empty URI and is not inline that are in an inbox
        //and its message is loaded complete
        public static final String PRECACHE_INBOX_SELECTION_NOT_INLINE =
                PRECACHE_SELECTION_NOT_INLINE + " AND " + AttachmentColumns.MESSAGE_KEY + " IN ("
                +     "SELECT " + MessageColumns._ID + " FROM " + Message.TABLE_NAME
                +     " WHERE " + Message.ALL_INBOX_COMPLETE_SELECTION
                +     ")";

     // All attachments with an empty URI and contentID is not null, regardless of mailbox
        public static final String PRECACHE_SELECTION_INLINE =
                AttachmentColumns.CONTENT_URI + " isnull AND " + AttachmentColumns.FLAGS + "=0"
                + " AND " + AttachmentColumns.CONTENT_ID + " is not null";
        // Attachments with an empty URI and is inline that are in an inbox
        //and its message is loaded complete
        public static final String PRECACHE_INBOX_SELECTION_INLINE =
                PRECACHE_SELECTION_INLINE + " AND " + AttachmentColumns.MESSAGE_KEY + " IN ("
                +     "SELECT " + MessageColumns._ID + " FROM " + Message.TABLE_NAME
                +     " WHERE " + Message.ALL_INBOX_COMPLETE_SELECTION
                +     ")";
        // TS: tao.gan 2015-08-12 EMAIL FEATURE-ID ADD_S
        // TS: xinlei.sheng 2016-3-3 EMAIL BUGFIX_1664255 ADD_S
        public static final String PRECACHE_SELECTION_NOT_DOWNLOADED_INLINE =
                PRECACHE_SELECTION_INLINE + " AND " + AttachmentColumns.ISINLINE + "=1";
    ...
    public static final class AccountInfo extends EmailContent implements
            AccountInfoColumns {
        ...
        public static final String[] CONTENT_PROJECTION = new String[] {
                RECORD_ID,
                AccountInfoColumns.ACCOUNT_ID,
                AccountInfoColumns.IS_USEPROXY,
                AccountInfoColumns.PROXY_ADDRESS,
                AccountInfoColumns.PROXY_PORT,
                AccountInfoColumns.PROXY_USERNAME,
                AccountInfoColumns.PROXY_USERPASS
        };
        ...
        public static Uri insertAccountInfo(Context context, long accountId,
                String protocal, int nDownloadLimit) {
            ContentValues cv = new ContentValues();
            cv.put(AccountInfoColumns.ACCOUNT_ID, accountId);
            return context.getContentResolver().insert(AccountInfo.CONTENT_URI,
                    cv);
        }

        public static Uri insertProxyInfo(Context context, long accountId,
                  boolean bUseProxy, String proxyAddr, int ProxyPort,
        ...
        public static boolean querryProxyInfo(Context context, long accountId,
                Account account) {
            Cursor c = context.getContentResolver().query(
                    AccountInfo.CONTENT_URI,
                    AccountInfo.CONTENT_PROJECTION,
                    AccountInfoColumns.ACCOUNT_ID + "="
                            + Integer.toString((int) accountId), null, null);
            try {
                if (c.moveToFirst()) {
                    account.mIsUseproxy = c.getInt(CONTENT_USE_PROXY_COLUMN) == 1 ? true
                            : false;
                    account.mProxyAddr = c.getString(CONTENT_PROXY_ADDR_COLUMN);
                    account.mProxyPort = c.getInt(CONTENT_PROXY_PORT_COLUMN);
                    account.mProxyUsername = c
                            .getString(CONTENT_PROXY_USERNAME_COLUMN);
                    account.mProxyUserpass = c
                            .getString(CONTENT_PROXY_USERPASS_COLUMN);
                    return true;
                } else {
        ...
        public static int updateProxyInfo(Context context, long accountId,
                boolean bUseProxy, String ProxyAddr, int ProxyPort,
                String Proxyname, String password) {
            ContentValues cv = new ContentValues();
            cv.put(AccountInfoColumns.IS_USEPROXY, bUseProxy ? 1 : 0);
            cv.put(AccountInfoColumns.PROXY_ADDRESS, ProxyAddr);
            cv.put(AccountInfoColumns.PROXY_PORT, ProxyPort);
            ...
            return context.getContentResolver().update(
                    ...
                    cv,
                    AccountInfoColumns.ACCOUNT_ID + "="
                            + Integer.toString((int) accountId), null);
        }
