...
public class Mailbox extends EmailContent implements EmailContent.MailboxColumns, Parcelable {
    ...
    /// TCT: can be used to identify all messages have be downloaded.
    ...
    //[FEATURE]-Add-BEGIN by TSNJ.Zhenhua.Fan,09/06/2014,FR 622609
    public static final String ORANGE_IMAP_SENTBOX_SERVER_ID = "INBOX/OUTBOX";
    public static final String ORANGE_IMAP_DRAFTBOX_SERVER_ID = "INBOX/DRAFT";
    public static final String ORANGE_IMAP_TRASHBOX_SERVER_ID = "INBOX/TRASH";
    //[FEATURE]-Add-END by TSNJ.Zhenhua.Fan
    //[FRETURE]-ADD-BEGIN by TSNJ.wei huang 10/27/2014 PR739323
    public static final String SFR_IMAP_SENTBOX_SERVER_ID = "OUTBOX";
    //[FEATURE]-ADD-END by TSNJ.wei huang
    //[FRETURE]-ADD-BEGIN by TSNJ.wei huang 10/27/2014 PR739323
    public static final String SFR_IMAP_DRAFTBOX_SERVER_ID = "DRAFT";
    //[FEATURE]-ADD-END by TSNJ.wei huang
    ...
    public static void resyncMailbox(final ContentResolver cr,
        ...
        if (cursor == null || cursor.getCount() == 0) {
            ...
            //[BUGFIX]-Add-BEGIN by TCTNB.caixia.chen,01/07/2015,PR 893304
            if (cursor != null) {
                cursor.close();
            }
    ...
    //[FEATURE]-Add-BEGIN by TSNJ.Zhenhua.Fan,09/06/2014,FR 622609
    public static long getOrangeImapDraftboxId(Context context, long accountId) {
        long mailboxId = NO_MAILBOX;
        Cursor c = context.getContentResolver().query(CONTENT_URI, ID_PROJECTION,
                PATH_AND_ACCOUNT_SELECTION, new String[] {ORANGE_IMAP_DRAFTBOX_SERVER_ID,Long.toString(accountId)}, null);
        if (c != null) {
            try {
                if (c.moveToFirst()) {
    ...
    public static long getOrangeImapSentboxId(Context context, long accountId) {
        long mailboxId = NO_MAILBOX;
        Cursor c = context.getContentResolver().query(CONTENT_URI, ID_PROJECTION,
                PATH_AND_ACCOUNT_SELECTION, new String[] {ORANGE_IMAP_SENTBOX_SERVER_ID,Long.toString(accountId)}, null);
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    mailboxId = c.getLong(ID_PROJECTION_COLUMN);
    ...
    public static long getOrangeImapTrashboxId(Context context, long accountId) {
        long mailboxId = NO_MAILBOX;
        Cursor c = context.getContentResolver().query(CONTENT_URI, ID_PROJECTION,
                PATH_AND_ACCOUNT_SELECTION, new String[] {ORANGE_IMAP_TRASHBOX_SERVER_ID, Long.toString(accountId)}, null);
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    mailboxId = c.getLong(ID_PROJECTION_COLUMN);
    ...
    public static boolean isOrangeImapDraftbox(Context context, long mailboxId) {
        boolean isDraft = false;
        Cursor c = null;
        try {
            c = context.getContentResolver().query(Mailbox.CONTENT_URI, new String[]{MailboxColumns.SERVER_ID},
                    MailboxColumns.ID + "=" + mailboxId, null, null);
            if (c != null && c.moveToFirst()) {
                String serverId = c.getString(0);
                if ((serverId != null) && serverId.equals(ORANGE_IMAP_DRAFTBOX_SERVER_ID)) {
                    isDraft = true;
                }
            }
        } catch(Exception e) {
    ...
    public static boolean isOrangeImapDraftboxOrTrashbox(Context context, long mailboxId) {
        boolean isDraftOrTrash = false;
        Cursor c = null;
        try {
            c = context.getContentResolver().query(Mailbox.CONTENT_URI, new String[]{MailboxColumns.SERVER_ID},
                    MailboxColumns.ID + "=" + mailboxId, null, null);
            if (c != null && c.moveToFirst()) {
                String serverId = c.getString(0);
                if ((serverId != null) && serverId.equals(ORANGE_IMAP_DRAFTBOX_SERVER_ID) || serverId.equals(ORANGE_IMAP_TRASHBOX_SERVER_ID)) {
                    isDraftOrTrash = true;
                }
            }
        } catch(Exception e) {
        ...
        return isDraftOrTrash;
    }
    //[FEATURE]-Add-END by TSNJ.Zhenhua.Fan
    //[FRETURE]-ADD-BEGIN by TSNJ.wei huang 10/27/2014 PR739323
    public static long getSfrImapSentboxId(Context context, long accountId) {
        long mailboxId = NO_MAILBOX;
        Cursor c = context.getContentResolver().query(CONTENT_URI, ID_PROJECTION,
                PATH_AND_ACCOUNT_SELECTION, new String[] {SFR_IMAP_SENTBOX_SERVER_ID,Long.toString(accountId)}, null);
        if (c != null) {
            try {
                if (c.moveToFirst()) {
                    mailboxId = c.getLong(ID_PROJECTION_COLUMN);
    ...
    //[FEATURE]-ADD-END by TSNJ.wei huang
    //[FRETURE]-ADD-BEGIN by TSNJ.wei huang 10/27/2014 PR739348
    public static boolean isSfrImapDraftbox(Context context, long mailboxId) {
        boolean isDraft = false;
        Mailbox mailbox = Mailbox.restoreMailboxWithId(context, mailboxId);
        if (mailbox != null && Account.isSfrImapAccount(context, mailbox.mAccountKey)) {
            if (SFR_IMAP_DRAFTBOX_SERVER_ID.equals(mailbox.mServerId)) {
               isDraft = true;
            }
        }
        return isDraft;
    ...
    /**
     * TCT: Update mailbox flag witch identified whether load all messages or not
