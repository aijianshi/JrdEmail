 ...
 *============== ============ =============== ==============================
 *CONFLICT-20001 2014/11/04   wenggangjin	  Modify the package conflict
 *CONFLICT-20001 2014/11/04   zhaotianyong	  Modify the package conflict
 *BUGFIX-933164  2015-02-16   zheng.zou       [REG][Force close][Email]Forceclose when print email from outbox of TCL email account
...
import com.tct.emailcommon.utility.AddressUtils;
import com.tct.fw.google.common.annotations.VisibleForTesting;
import com.tct.mail.utils.DecoderUtil;
import com.tct.mail.utils.EncoderUtil;
...
public class Address implements Parcelable {
    ...
    public static synchronized Address getEmailAddress(String rawAddress) {
        if (TextUtils.isEmpty(rawAddress)) {
            // TS: zheng.zou 2015-02-16 EMAIL BUGFIX_933164 MOD_S
            // do not return null to avoid Exception
            return new Address("","");
            // TS: zheng.zou 2015-02-16 EMAIL BUGFIX_933164 MOD_E
        }
        String name, address;
    ...
    public static Address[] parseIgnoreValid(String addressList) {
    ...
    public void writeToParcel(Parcel out, int flags) {
        ...
        out.writeString(mAddress);
            ...
            /**
             * TCT: A light-weight method to get first email address from a given string.
             * @param packedString string packed with pack() or CSV of RFC addresses
    ...
    public static String getFirstMailAddress(String packedString) {
        ...
        return packedString;
            ...
            /** TCT: Rollback to ICS's method to well support non-ascii encoded local search
    ...
    /**
     * Get string array from addressList string(The method is similar with fromHeader() method)
     * @param addressList a CSV of RFC822 addresses or the deprecated legacy string format
     * @return array of string parsed from <code>addressList</code>
     */
    public static String[] getMailsArray(String addressList) {
        if (addressList == null || addressList.length() == 0) {
            return new String[0];
        }
        // IF we're CSV, just parse
        if ((addressList.indexOf(LIST_DELIMITER_PERSONAL) == -1) &&
                (addressList.indexOf(LIST_DELIMITER_EMAIL) == -1)) {
            return Address.mailsParse(addressList);
        }
        // Otherwise, do backward-compatible unpack
        ...
        */
        ...
        while (pairStartIndex < length) {
            ...
            if (addressEndIndex == -1 || pairEndIndex <= addressEndIndex) {
                // in this case the DELIMITER_PERSONAL is in a future pair,
                // so don't have personal, and don't update addressEndIndex
    ...
    /**
     * Parse a comma-delimited list of addresses in RFC822 format and return an
     * array of string.((The method is similar with parse() method))
     *
     * @param addressList Address list in comma-delimited string.
     * @return An array of 0 or more string.
     */
    private static String[] mailsParse(String addressList) {
        if (addressList == null || addressList.length() == 0) {
            return new String[0];
        }
        Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(addressList);
    ...
    /**
     * To Parse email address in the MessageHeaderView
     */
    public static String[] addressParse(String[] addressList){
        int length =addressList.length;
        if (addressList == null || length == 0) {
            return new String[0];
        }
        ArrayList<String> addresses = new ArrayList<String>();
        for(int i = 0; i < length; i++){
            Rfc822Token[] tokens = Rfc822Tokenizer.tokenize(addressList[i]);
            for(int x = 0, len = tokens.length; x < len; ++x){
                Rfc822Token token = tokens[x];
                String address = token.getAddress();
                if (!TextUtils.isEmpty(address) && isValidAddress(address)) {
                    addresses.add(address);
                }
