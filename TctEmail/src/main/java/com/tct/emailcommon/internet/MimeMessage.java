...
public class MimeMessage extends Message {
    ...
    public boolean hasNoHeader() {
        return mHeader == null || mHeader.mFields.size() == 0;
    }
    ...
    public int getPriority() throws MessagingException {
        int i = 1;//default is 1 normal
        // We use one flag to indicate "Importance", "Priority" and "X-Priority"
        // Because they have the similar meaning
        // Usually one message should includes only one of three

        // Get importance header
        // 0:high ; 1:normal ; 2:low
        String importance = getFirstHeader("Importance");
        if (null != importance && !"".equals(importance)) {
            if ("High".equalsIgnoreCase(importance)) {
                i = EmailContent.Message.FLAG_PRIORITY_HIGH;
            } else if ("Low".equalsIgnoreCase(importance)) {
                i = EmailContent.Message.FLAG_PRIORITY_LOW;
            }
            return i;
        }

        // Get priority header
        // 0:Urgent ; 1:normal ; 2:non-urgent
        String priority = getFirstHeader("Priority");
        if (null != priority && !"".equals(priority)) {
            if ("Urgent".equalsIgnoreCase(priority)) {
                i = EmailContent.Message.FLAG_PRIORITY_HIGH;
            } else if ("non-urgent".equalsIgnoreCase(priority)) {
                i = EmailContent.Message.FLAG_PRIORITY_LOW;
            }
            return i;
        }

        // Get X-priority header
        // The X-Priority value ranges from 1 to 5, with 1 being
        // high priority, 3 is normal, and 5 is low.
        //  "2" and "4" are in-between, but "1" or "3" used more
        // We consider "2" equal with "1",  "4" equal with "5"
        // Including extra text, such as "1 (High)", we get the
        // numbers only.
        String xpriority = getFirstHeader("X-Priority");
        if (null != xpriority && !"".equals(xpriority)) {
            int xpriorityIdx = Integer.parseInt(xpriority.substring(0, 1));
            if (xpriorityIdx < 3) {
                i = EmailContent.Message.FLAG_PRIORITY_HIGH;
            } else if (xpriorityIdx > 3) {
                i = EmailContent.Message.FLAG_PRIORITY_LOW;
            }
            return i;
        }
     ...
     * @throws MessagingException
      */
    public void setPriority(int priorityInt) throws MessagingException {
        // 0:high/urgent ; 1:normal ; 2:low/non-urgent
        // not necessary to add header for normal
        // the recipient client will treat it as normal if no header
        if (priorityInt == EmailContent.Message.FLAG_PRIORITY_HIGH) {
            setHeader("Importance", "High");
            setHeader("Priorty", "urgent");
        } else if (priorityInt == EmailContent.Message.FLAG_PRIORITY_LOW) {
            setHeader("Importance", "low");
            setHeader("Priorty", "non-urgent");
        }
    }
    // [FEATURE]-ADD-END by TSNJ,zhujian.shao,10/23/2014,FR-736411
