...
public class EmailServiceProxy extends ServiceProxy implements IEmailService {
    ...
    public Bundle validate(final HostAuthCompat hostAuthCom) throws RemoteException {
        setTask(new ProxyTask() {
            @Override
            public void run() throws RemoteException {
                // TS: tao.gan 2015-11-11 EMAIL BUGFIX-1111005 MOD_S
                try {
                    mReturn = mService.validate(hostAuthCom);
                } catch (IllegalArgumentException e) {
                    LogUtils.e(TAG, "IllegalArgumentException while validate hostAuth : " + e.getMessage());
                } catch (SecurityException e) {
                    LogUtils.e(TAG, "SecurityException while validate hostAuth : " + e.getMessage());
                }
                // TS: tao.gan 2015-11-11 EMAIL BUGFIX-1111005 MOD_E
            }
        }, "validate");
        waitForCompletion();
        if (mReturn == null) {
    ...
    public Bundle syncOof(final long accountId, final String command, final Bundle content)
        ...
        setTask(new ProxyTask() {
            @Override
            public void run() throws RemoteException {
                mReturn = mService.syncOof(accountId, command, content);
            }
        }, "syncOof");
        waitForCompletion();
     ...
     */
    @Override
    public int fetchMessage(final long messageId) throws RemoteException {
        setTask(new ProxyTask() {
            @Override
            public void run() throws RemoteException {
                mReturn = mService.fetchMessage(messageId);
            }
        }, "fetchMessage");
        waitForCompletion();
        if (mReturn == null) {
            // This occurs if there is a timeout or remote exception. Is not expected to happen.
            LogUtils.wtf(TAG, "failed to fetch message");
            return 0;
        } else {
            return (Integer) mReturn;
        }
